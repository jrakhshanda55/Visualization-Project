...
gnn_controls = dbc.Accordion([
    dbc.AccordionItem([
        ...
        dcc.Slider(id="epochs", min=30, max=100, step=10, value=50,
...
content = html.Div([
    ...
    html.P("Edge thickness represents number of dependencies; arrows indicate direction of dependency.",
           className="text-muted text-center", style={"fontSize": "16px", "fontStyle": "italic"}),
    html.Hr(),
...
@app.callback(
    ...
    State("deps-store", "data"),
    prevent_initial_call=False
)
def build_eda(_trigger, dataset_name, topn, nodes_records, deps_records):
    """Simplified and clean EDA (consistent naming)."""
    ...
    if "Dependency_Type" in deps_df.columns:
        dep_counts = (
    ...
    # --- Module distribution ---
    if "Module" in nodes_df.columns and num_files:
        mod_counts = (
            nodes_df["Module"]
            .astype(str)
            .value_counts()
    ...
    try:
        deps_for_plot = deps_df.rename(columns={"Dependency_Type": "Type", "Count": "Dependency_Count"})
        implemented_fig = plot_implemented_architecture(nodes_df, deps_for_plot)
    ...
    try:
        base_name = Path(DATASETS[dataset_name][0]).stem
...
def train_embeddings(n_clicks, ds_name, selected_types, feat_mode, enc_type,
    ...
    fig.update_traces(marker=dict(size=20, opacity=0.99,
